...
#define LSLaunchADBottomHeight 90
...

{
 ...
 // adImageView.contentMode = UIViewContentModeScaleAspectFill;
  ...
  {
    imageViewHeight = viewSize.height - LSLaunchADBottomHeight*SCREEN_POINT;
  }
  ...
  if (showSkipButton) {
    ...
    skipButton.frame =
        CGRectMake(viewSize.width - 20 - width, 20, width, height);
...
- (void)autoCount {
  ...
  if (self.skipBtn && self.showCountTimeOfButton) {
    if (self.adTime == 1) {
      [self.skipBtn
          setTitle:[NSString stringWithFormat:@"跳过 %d", self.adTime]
